...
def classifyImage(input_image):
    ...
    top_predictions = [(class_names[i], predictions[i] * 100)
...
def display_response(response):
    ...
    st.write(", ".join([ingredient.capitalize()
...
st.sidebar.write(
    "Upload an image and/or enter a query to get started! Explore our trained dish types listed below for guidance.")

uploaded_image = st.sidebar.file_uploader("Choose an image:", type="jpg")
query = st.sidebar.text_area("Enter your query:", height=100)
recipe_submit = st.sidebar.button(
    label='Chain Recipe', icon=':material/link:', use_container_width=True)
...
if uploaded_image and recipe_submit:
    ...
    st.warning("Please input an image and/or a prompt.",
